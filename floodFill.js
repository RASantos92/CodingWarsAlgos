...
var testArr = [11,2,5,4,9,6,7]
function recursiveSelectionSort(arr, i = 0, j=i+1, min = i){
    if(i === arr.length-1) return i
    if(arr[j]< arr[min]) min = j
    if(j === arr.length-1){
        if(arr[i]>arr[min]) [arr[min],arr[i]] = [arr[i],arr[min]]
        else min++
        recursiveSelectionSort(arr,i+1,j=i+1,min)
    } 
    else recursiveSelectionSort(arr,i,j+1,min)
}
console.log(recursiveSelectionSort(testArr))
console.log(testArr)